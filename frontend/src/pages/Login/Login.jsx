...
export default function Login() {
  ...
  // track selected role in a ref to avoid closure capture issues in callbacks
  const roleRef = useRef(role);
  useEffect(() => {
    roleRef.current = role;
  }, [role]);

  // handler when user clicks sign in with google
  const handleGoogleClick = async () => {
    try {
      ...
      // perform firebase google popup sign-in
      await loginWithGoogle(roleRef.current);
      
      setIsSubmitting(false);
      navigate('/'); // redirect to marketplace home page on success
    } catch (error) {
  ...
  // handler when user submits standard email/password login form
